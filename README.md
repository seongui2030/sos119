...
이 문서는 `sos119` 프로젝트의 소스코드를 처음 배우는 고등학생도 이해할 수 있도록 설명하는 작은 교과서입니다. 이 프로젝트는 음성으로 건강 상태를 확인하고, 끝말잇기 게임으로 어르신의 인지활동을 돕는 웹 애플리케이션입니다.

프로젝트의 목표는 유엔 지속가능발전목표(SDGs) 중 **목표 3: 건강과 웰빙**에 연결됩니다. 초고령화사회에서는 몸의 건강뿐 아니라 대화, 기억, 집중과 같은 일상적인 인지활동도 중요합니다. `똑똑 비서`는 전문 의료진을 대신하지 않지만, 말하기와 단어 놀이를 쉽게 시작하도록 도와주는 보조 도구입니다.
